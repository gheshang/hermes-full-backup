...
[步骤3] 图结构解析 → 从变压器节点向上回溯，构建完整层级路径
    ↓
[步骤4] 联网检索 → 查询该变电站/线路的官方资料（容量、投运时间、所属供电局等）
    ↓
...
| 难点 | 说明 | 解决思路 |
|------|------|----------|
| 单线图识别精度 | 电力单线图符号复杂（变压器、断路器、隔离开关等专用符号），通用OCR无法识别 | 多模态VLM（Qwen2.5-VL/InternVL2）+ 电力符号微调提示词 |
| 拓扑关系提取 | 图中设备用线条连接，需识别「谁连谁」的层级关系 | VLM输出结构化JSON（节点+边），或用Segment Anything做轮廓分割后OCR |
| 路径回溯逻辑 | 从变压器向上找杆塔→线路→变电站，需理解电力系统命名规范 | 在Dify中用Code节点写回溯算法，配合规则引擎 |
| 联网检索准确性 | 变电站名称可能存在别名/简称，需模糊匹配 | 用Dify的HTTP Request节点调用电力数据API或搜索引擎 |
...
│         ┌──────────────┐                                         │
│         │  LLM节点1     │  ← 多模态识别单线图                     │
│         │ (Qwen2.5-VL)  │  输入: 图像 + 提示词                    │
│         └──────┬───────┘  输出: 设备节点列表 + 连接关系JSON       │
│                │                                                  │
│                ▼                                                  │
│         ┌──────────────┐                                         │
│         │ Code节点1     │  ← 路径回溯算法                         │
│         │ (Python)      │  输入: 节点+边JSON + 目标变压器名       │
│         └──────┬───────┘  输出: {substation, line, pole, tx}     │
│                │                                                  │
│                ▼                                                  │
│         ┌──────────────┐                                         │
│         │ HTTP Request  │  ← 联网检索变电站/线路资料               │
│         │ 节点          │  输入: 变电站名 + 线路名                 │
│         └──────┬───────┘  输出: 容量/投运时间/所属单位等           │
│                │                                                  │
│                ▼                                                  │
│         ┌──────────────┐                                         │
│         │ LLM节点2      │  ← 数据整合 + 填报表单生成               │
│         │ (DeepSeek)    │  输入: 路径信息 + 检索结果               │
│         └──────┬───────┘  输出: 结构化填报JSON                    │
│                │                                                  │
│                ▼                                                  │
│         ┌──────────────┐                                         │
│         │ End节点       │  输出: 填报结果 + 人工审核标记            │
│         └──────────────┘                                         │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
子agent验证结论（因搜索超时未完成完整报告，以下为基于已有信息的结论）：
