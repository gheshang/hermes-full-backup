...
*   **Configuration Methods:**
    *   Environment variables for individual users.
    *   Managed settings file for administrator-controlled, organization-wide deployment.
*   **Exporter Types:** `otlp`, `prometheus`, `console` for metrics; `otlp`, `console` for logs.
*   **OTLP Configuration:** Endpoint, protocol (gRPC, HTTP/JSON, HTTP/Protobuf), and authentication headers.
*   **Export Intervals:** Customizing frequency for metrics and logs.
*   **Metrics Cardinality Control:** Managing attributes like `session.id`, `app.version`, and `user.account_uuid`.
*   **Available Metrics:** Specific metrics exported by Claude Code (e.g., `claude_code.session.count`, `claude_code.token.usage`).
*   **Available Events:** Specific events exported by Claude Code (e.g., `claude_code.session.start`, `claude_code.code_edit_tool.decision`).

### 3. Important Technical Details

*   **Telemetry Enablement:** Requires `CLAUDE_CODE_ENABLE_TELEMETRY=1`.
*   **Exporters:** Users can choose `otlp` (OpenTelemetry Protocol), `prometheus` (metrics only), or `console` for debugging. Multiple exporters can be comma-separated.
*   **OTLP Endpoint:** Configured via `OTEL_EXPORTER_OTLP_ENDPOINT` (e.g., `http://localhost:4317`). Specific endpoints for metrics (`OTEL_EXPORTER_OTLP_METRICS_ENDPOINT`) and logs (`OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`) can override the general one.
*   **Protocols:** OTLP supports `grpc`, `http/json`, and `http/protobuf`.
*   **Authentication:** `OTEL_EXPORTER_OTLP_HEADERS` for bearer tokens, and mTLS options (`OTEL_EXPORTER_OTLP_METRICS_CLIENT_KEY`, `OTEL_EXPORTER_OTLP_METRICS_CLIENT_CERTIFICATE`).
*   **Export Intervals:** `OTEL_METRIC_EXPORT_INTERVAL` (default 60s) and `OTEL_LOGS_EXPORT_INTERVAL` (default 5s). Shorter intervals are useful for debugging.
*   **Administrator Configuration:** Managed settings files (`/Library/Application Support/ClaudeCode/managed-settings.json` on macOS, `/etc/claude-code/managed-settings.json` on Linux) provide centralized control. These settings have high precedence and cannot be overridden by user environment variables.
*   **Metrics Cardinality:** Environment variables like `OTEL_METRICS_INCLUDE_SESSION_ID` allow control over which attributes are included in metrics, impacting storage and query performance.
*   **Sensitive Data:** `OTEL_LOG_USER_PROMPTS` is disabled by default, indicating a consideration for user privacy regarding prompt content.
*   **Metrics List:** Includes operational metrics (session count, lines of code, PRs, commits) and usage/cost metrics (token usage, cost usage).
*   **Event List:** Includes lifecycle events (session start/end) and specific tool decisions (code edit tool).
...
**Administrator Managed Settings (JSON):**
...
**Example Configurations (various scenarios):**
...
*   **OpenTelemetry (OTel):** Fundamental understanding of OTel concepts, including metrics, logs/events, exporters, and the OTLP protocol.
*   **Observability Platforms:** Knowledge of how to set up and configure metrics and logs backends (e.g., Prometheus, Grafana, Jaeger, ELK stack, commercial APM tools) to receive and visualize OTel data.
*   **Environment Variables:** Basic understanding of setting and managing environment variables in a shell.
*   **JSON Configuration:** Familiarity with JSON syntax for administrator settings.
*   **System Administration (for managed settings):** Knowledge of file system paths on macOS and Linux, and potentially Mobile Device Management (MDM) solutions for large-scale deployments.
*   **Network Configuration:** Understanding of endpoints, ports, and network protocols (gRPC, HTTP) for OTLP communication.
*   **Security:** Awareness of authentication methods (bearer tokens, mTLS) for secure data export.
*   **Data Cardinality:** Understanding the impact of high cardinality on monitoring system performance and cost.
*   **Claude Code Usage:** Basic familiarity with running and interacting with the Claude Code application.
```
