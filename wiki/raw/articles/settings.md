...
`permissions`| `allow` and `deny` keys are a list of [permission rules](/_sites/docs.anthropic.com/en/docs/claude-code/settings#permissions)| `{"allow": [ "Bash(npm run lint)" ]}`  
...
A rule that is just the tool name matched any use of that tool. For example, adding `Bash` to the list of allow rules would allow Claude Code to use the Bash tool without requiring user approval. See the list of [tools available to Claude](security#tools-available-to-claude).
...
If you prefer to disable the auto-updater instead of fixing permissions, you can set the `DISABLE_AUTOUPDATER` [environment variable](/_sites/docs.anthropic.com/en/docs/claude-code/settings#environment-variables) to `1`
...
*   Hierarchical settings files: User, Project (shared and local), and Enterprise policies.
*   Available configuration options within `settings.json`.
*   Settings precedence rules.
*   Detailed explanation of permission rules (`allow`, `deny`) for various tools (Bash, Read, Edit, WebFetch, MCP).
*   Managing auto-updater permissions and troubleshooting.
*   Optimizing terminal setup for Claude Code, including line breaks and notifications.

### 3. Important Technical Details

*   **Configuration Mechanism:** `settings.json` files are the primary mechanism, supplemented by the `/config` interactive command.
*   **Settings File Locations:**
    *   User: `~/.claude/settings.json`
    *   Shared Project: `.claude/settings.json`
    *   Local Project: `.claude/settings.local.json` (git-ignored)
    *   Enterprise Policies: macOS: `/Library/Application Support/ClaudeCode/policies.json`, Linux/WSL: `/etc/claude-code/policies.json`
*   **Settings Precedence (Highest to Lowest):** Enterprise policies > Command line arguments > Local project settings > Shared project settings > User settings.
*   **Permissions:**
    *   Managed via `/permissions` UI.
    *   `allow` rules grant access without approval.
    *   `deny` rules prevent access and take precedence over `allow`.
    *   Format: `Tool(optional-specifier)`.
    *   **Tool-specific specifiers:**
        *   `Bash(command)`: Exact match or prefix match (`Bash(npm run test:*)`).
        *   `Read`/`Edit(path)`: Follows `.gitignore` spec, supports relative, home (`~/`), and absolute (`//`) paths.
        *   `WebFetch(domain:example.com)`: Restricts web fetches to specific domains.
        *   `MCP(server__tool)`: For custom tools provided by servers.
*   **Auto-updater:** Requires write permissions to global npm prefix. Recommended solution involves creating a user-writable npm prefix. Can be disabled via `DISABLE_AUTOUPDATER` environment variable.
*   **Terminal Optimization:**
    *   Supported shells: Bash, Zsh, Fish.
    *   Line breaks: `\` + Enter or Option/Meta + Enter (requires terminal configuration).
    *   Notifications: Terminal bell notifications.
...
```bash
# Recommended auto-updater setup: Create a new user-writable npm prefix
npm list -g --depth=0 > ~/npm-global-packages.txt
mkdir -p ~/.npm-global
npm config set prefix ~/.npm-global
echo 'export PATH=~/.npm-global/bin:$PATH' >> ~/.bashrc # Adjust for your shell
source ~/.bashrc
npm install -g @anthropic-ai/claude-code
# Optional: Reinstall previous global packages
# npm install -g package1 package2 package3...
```

### 5. Related Concepts or Prerequisites

*   **Command Line Interface (CLI) usage:** Familiarity with running commands in a terminal.
*   **JSON format:** Understanding of JSON syntax for configuration files.
*   **Environment Variables:** Knowledge of how environment variables work and are set.
*   **File System Navigation:** Understanding of file paths, home directories (`~`), and absolute paths.
*   **Git and `.gitignore`:** Basic understanding of Git and how `.gitignore` patterns work for file matching.
*   **npm (Node Package Manager):** Familiarity with `npm` for package installation and global prefixes, especially for auto-updater management.
*   **Shell Configuration:** Knowledge of configuring shell profiles (e.g., `.bashrc`, `.zshrc`) for PATH modifications.
*   **Terminal Emulators:** Basic understanding of terminal application settings (e.g., for configuring Meta key).
*   **Security Best Practices:** Awareness of why limiting tool permissions and using user-writable directories is important.
```
