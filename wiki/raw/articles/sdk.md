...
*   **Authentication:** How to set up access to the Claude Code SDK.
*   **Basic Usage:** Running single prompts, piping input, and controlling output format.
*   **Advanced Usage:**
    *   Multi-turn conversations (continuing/resuming sessions).
    *   Customizing Claude's behavior with system prompts.
    *   Model Context Protocol (MCP) for extending capabilities with external tools.
    *   Custom permission prompt tools for controlling tool invocation.
*   **CLI Options:** A table summarizing key command-line flags.

### 3. Important Technical Details

*   **Current SDK State:** Primarily command-line interface (CLI) based; TypeScript and Python SDKs are "coming soon."
*   **Authentication Method:** Requires an `ANTHROPIC_API_KEY` environment variable, obtained from the Anthropic Console. Secure storage (e.g., GitHub secrets) is recommended.
*   **Non-interactive Mode:** The `-p` or `--print` flag is crucial for programmatic use, enabling output to stdout.
*   **Output Formats:** Supports `text`, `json`, and `stream-json` via `--output-format`.
*   **Conversation Management:**
    *   `--continue` (or `-c`): Continues the most recent conversation.
    *   `--resume <session_id>` (or `-r`): Resumes a specific conversation by ID.
*   **System Prompts:**
    *   `--system-prompt`: Overrides the default system prompt. Only works with `--print`.
    *   `--append-system-prompt`: Appends instructions to the default system prompt. Only works with `--print`.
*   **Model Context Protocol (MCP):**
    *   Allows extending Claude Code with external tools (e.g., filesystem, GitHub).
    *   Requires a JSON configuration file specifying `mcpServers`.
    *   **Security:** Tools must be explicitly allowed using `--allowedTools`. Tool names follow `mcp__<serverName>__<toolName>`.
*   **Custom Permission Prompt Tool:**
    *   Uses an MCP tool (`--permission-prompt-tool`) to handle tool invocation permissions.
    *   Invoked when `settings.json`, `--allowedTools`, or `--disallowedTools` don't explicitly allow/deny a tool call.
    *   The MCP tool must return a JSON-stringified payload with `behavior: "allow"` or `behavior: "deny"`.
    *   Can optionally return `updatedInput` if the tool modified the input.
...
**Custom Permission Prompt Tool (TypeScript Example):**
...
*   **Anthropic API Key:** Fundamental for authentication.
*   **Environment Variables:** Understanding how to set and manage them for API keys.
*   **Command Line Interface (CLI):** Familiarity with executing commands and using flags/arguments.
*   **JSON Format:** Understanding JSON for output and configuration files (MCP).
*   **Subprocesses:** Basic understanding of how applications can run other programs.
*   **AI/LLM Concepts:** General understanding of how large language models like Claude operate, especially regarding prompts and system prompts.
*   **Security Best Practices:** Awareness of secure storage for API keys (e.g., not hardcoding them).
*   **Model Context Protocol (MCP):** Implies a broader ecosystem for extending AI model capabilities, potentially involving external servers and tool definitions.
*   **TypeScript/Python:** While the SDK is currently CLI-focused, the mention of upcoming SDKs implies future relevance of these languages.
