...
### Mode A+ · 第三方 OpenAI 兼容 API

**触发条件**：`OPENAI_IMAGE_BASE_URL` 指向第三方兼容网关（如 SenseNova、DeepSeek、Moonshot 等），且有对应 API Key。

**行为**：与 Mode A 类似，但需注意：
- **尺寸限制**：第三方 API 支持的尺寸可能与 OpenAI 不同，必须先查文档（见 `references/provider-quirks.md`）
- **响应格式**：优先使用 `response_format: "b64_json"`，避免 URL 签名过期问题
- **模型名**：使用提供商指定的模型 ID（如 `sensenova-u1-fast`）
...
第三方 API（如 SenseNova）返回的 `url` 字段通常有时效性（1-2 小时）。**不要依赖 URL 下载图片**，签名会过期。
...
强制返回 base64 编码的图片数据，可立即保存，无过期问题。

### 陷阱 2：尺寸限制不统一

OpenAI 标准尺寸（`1024x1024`, `1024x1792` 等）在第三方 API 上可能不支持。

**正确做法**：
1. 先查提供商文档（见 `references/provider-quirks.md`）
2. 或使用 `scripts/check-sizes.js` 探测支持的尺寸
3. 常见第三方尺寸：
   - SenseNova U1 Fast: `1344x3136`, `1536x2752`, `2048x2048` 等
   - DeepSeek: 类似 OpenAI
   - Moonshot: 需查文档

### 陷阱 3：模型名不通用

`gpt-image-2` / `dall-e-3` 是 OpenAI 模型名。第三方 API 使用自己的模型 ID。
...
- `scripts/check-mode.js`：**先跑这个**，检测运行模式（A / B / C / A+）
- `scripts/generate.js`：文本生图（仅 Mode A / A+ 使用）
- `scripts/edit.js`：基于原图 / 遮罩改图（仅 Mode A 使用）
- `scripts/shared.js`：共享请求、保存、环境变量读取逻辑
- `references/`：分层结构化提示词模板 + 提供商 quirks
  - `references/prompt-writing.md` — 模板方法论总规范
  - `references/provider-quirks.md` — **第三方 API 特殊限制**（必读）（A / B / C 三模式都用）
...
- `references/prompt-writing.md` — 提示词构造方法论
- `references/image-to-prompt-reverse-engineering.md` — **从图像反推 prompt 的技术**
...
- 你需要把案例抽象成可复用模板
- **用户要求分析某张图片的生成提示词** → 用 image-to-prompt-reverse-engineering
