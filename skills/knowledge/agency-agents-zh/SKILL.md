...
description: 中文 AI Agent 角色库 — 212 个专业角色提示词，覆盖 18 个部门。通过文件路径加载任意角色，快速切换专业视角。
color: purple
---

# Agency Agents 中文角色库

本地仓库路径: `~/agency-agents-zh/`

## 用法

当用户需要某个专业角色时，读取对应的 .md 文件内容，将其作为系统提示词注入到当前对话中。

**调用方式：**
1. 用户说出角色中文名（如"软件架构师"），从下方索引找到文件路径
2. 用 `read_file` 读取 `~/agency-agents-zh/{文件路径}` 的完整内容
3. 按该角色的规则、风格和框架来完成任务

**示例：** 用户说"用软件架构师来评审我的架构" -> 读取 `engineering/engineering-software-architect.md` -> 按角色行事
...
1. 当用户请求某个角色（中文名或部门），在索引中查找对应文件路径
2. 读取 `~/agency-agents-zh/{文件路径}` 获取完整提示词
3. 严格按该角色的身份、规则、框架和沟通风格来完成任务
4. 在回复开头声明角色身份，让用户知道你正在扮演哪个专家

## 注意事项

- 每个角色文件包含 YAML frontmatter（name/description/color）和完整的角色提示词
- 可以同时组合多个角色（如"用软件架构师+安全工程师来评审"）
- 如果用户没指定角色但任务明显属于某个领域，主动建议使用对应角色
- 仓库还包含 examples/ 和 strategy/ 目录，包含工作流示例和多角色协作方案
