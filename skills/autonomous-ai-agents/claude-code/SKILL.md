...
CC Switch 有三个版本，按场景选择：

| 版本 | 仓库 | 类型 | 适用 |
|------|------|------|------|
| CC Switch Desktop | farion1231/cc-switch | Tauri桌面应用 | macOS/Windows/Linux桌面 |
| **CC Switch CLI** | SaladDay/cc-switch-cli | **纯CLI，Rust编译** | **headless Linux ✅** |
| CC Switch Web | Laliet/cc-switch-web | Web服务器 | headless + 浏览器访问 |

### 安装（Linux headless — CC Switch CLI）
```bash
# 一键安装
curl -fsSL https://github.com/SaladDay/cc-switch-cli/raw/main/install.sh | bash

# 验证
cc-switch --version  # 当前 v5.3.4+
```
...
### 配置步骤（CLI版）
1. 添加Provider（交互式，需TTY）：
   ```bash
   cc-switch provider add -a claude
   # 按提示填写：Name, Base URL, API Key, Model
   ```
2. 启动本地Proxy：
   ```bash
   ...
   ```
3. 设置环境变量（写入 `~/.cc-switch/env.sh` 并 source 到 `.bashrc`）：
   ```bash
...
4. 开机自启（crontab）：
   ```bash
   (crontab -l; echo "@reboot /home/hangskf/.local/bin/cc-switch proxy serve --takeover claude >> ~/.cc-switch/proxy.log 2>&1") | crontab -
   ```

### 踩坑记录

1. **API Key脱敏问题**：Hermes框架会对写入config.yaml的key做脱敏（`sk-XDO...HI2b`），中划号是真实存储。添加provider必须通过交互式命令或python直接写SQLite数据库（`~/.cc-switch/cc-switch.db`）
2. **Anthropic→OpenAI协议转换**：CC Switch proxy核心功能就是做协议转换。Claude Code发Anthropic格式请求（`/v1/messages`），proxy转成OpenAI格式（`/v1/chat/completions`）再转发。**不能简单改ANTHROPIC_BASE_URL指向OpenAI兼容端点**
3. **模型名映射**：CC Switch用 `ANTHROPIC_DEFAULT_OPUS_MODEL`/`SONNET_MODEL`/`HAIKU_MODEL` 做模型名映射。Claude Code请求的claude-sonnet-4等模型名会被替换成你配的模型。直接传model字段可能被忽略
4. **默认端口**：CLI版proxy监听 `127.0.0.1:15721`（Desktop版是8080）
5. **proxy serve必须后台运行**：`cc-switch proxy serve --takeover claude` 是前台命令，需用crontab或systemd持久化
6. **SQLite数据库**：CC Switch CLI用SQLite存配置（`~/.cc-switch/cc-switch.db`），不认旧的JSON配置。非交互操作需直接写数据库：
   ```python
   import sqlite3, json
   conn = sqlite3.connect('/home/hangskf/.cc-switch/cc-switch.db')
   c = conn.cursor()
   c.execute('SELECT settings_config FROM providers WHERE id="your-provider-id"')
   config = json.loads(c.fetchone()[0])
   # 修改config...
   c.execute('UPDATE providers SET settings_config=? WHERE id="your-provider-id"', (json.dumps(config),))
   conn.commit()
   ```

### Token 消耗对比（关键发现）

| 方案 | 费用 vs Hermes直接做 | 结论 |
|------|---------------------|------|
| CC Switch → MiniMax | CC ≈ 0.2-0.5x | **CC更便宜！** |
| CC Switch → z-ai(GLM-5) | CC ≈ 0.4-0.7x | CC更便宜 |
| 直连 Claude Sonnet | CC ≈ 3-8x | Hermes更便宜 |

CC Switch 转发到便宜模型时，Claude Code 反而比 Hermes 直接做更省钱：
- 国内模型单价远低于 Sonnet（MiniMax 约 $0.15/M input vs Sonnet $3/M）
- Claude Code token 效率更高（子进程消化文件，input 少 40-60%）
- 两者叠加 → CC+便宜模型比 Hermes+GLM-5 省 50-80%

### 效率优势（非费用维度）

| 维度 | Claude Code | Hermes直接 | 胜者 |
|------|------------|-----------|------|
| 开发速度 | 快2-4x | 基准 | CC |
| 代码质量(同模型) | 多轮自纠更高 | 单次输出 | CC |
| 上下文容量 | 子进程独立 | 主上下文易溢出 | CC |
| 调试自主性 | 自循环 | 需人工反馈 | CC |

### 注意
- 便宜模型编码能力弱于 Sonnet，CC+便宜模型 < CC+Sonnet
- 但 CC 的 agentic loop 多轮自纠能弥补一部分差距

## Pitfalls & Gotchas

1. **Claude Code 模型不可用** — 所有模型别名（`claude-sonnet-4-6`、`claude-3-5-sonnet-20241022`、`haiku` 等）均报错 "model may not exist or you may not have access"。这是 Claude Code CLI 的模型别名与账户实际可用模型不匹配。**解决**：运行 `claude doctor` 检查健康状态，或直接用 `claude -p '...'` 不带 `--model` 参数（使用默认模型）。如果默认模型也失败，检查 `claude auth status` 确认 OAuth 登录是否有效。
