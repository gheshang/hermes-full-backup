...
## 自审报告 2026-05-05 — Skills + Config 检查

### ✅ 已自动执行

**Skills 结构检查**：
- 总计 166 个 skill（80 local, 85 builtin, 1 hub-installed）
- 扫描所有 local/builtin skill 目录的 SKILL.md → 全部存在且非空，无缺失
- 空 SKILL.md：0 个

**Config 审计**：
- SOUL.md：存在且非空 ✅
- config.yaml 关键段（platforms / custom_providers / model / memory）：全部存在 ✅
- config.yaml.bak 存在，当前 446 行/9111 字节 vs 备份 360 行/7876 字节 → 差异在 20% 以内 ✅
- 含 "deprecated" 的模型配置：无 ✅

### ⚠️ 待确认

**Skills 问题**：
- **guizang-ppt-skill**：在 `hermes skills list` 中显示为 local+enabled，但 `~/.hermes/skills/` 下无对应目录。注册但文件缺失，属于幽灵 skill。
- **html-ppt**：同上，注册为 local+enabled，无目录。

→ `hermes skill_manage action="remove" name="guizang-ppt-skill"` （或确认是否需要重新安装）
→ `hermes skill_manage action="remove" name="html-ppt"` （或确认是否需要重新安装）

**Config 建议**：
- 无 critical 问题，config.yaml 结构完整且与备份一致性正常

### ℹ️ 状态概览

- Skills：166 个启用（80 local / 85 builtin / 1 hub），2 个幽灵（有注册无文件）
- Config：4 关键段完整，无 deprecated 模型，备份一致性 OK
- SOUL.md：正常
