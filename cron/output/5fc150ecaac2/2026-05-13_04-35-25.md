...
| 检查项 | 结果 |
|--------|------|
| `~/.hermes/SOUL.md` | ✅ 存在且非空 |
| `config.yaml` 含 deprecated 模型 | ✅ 无 |

#### Skills 结构检查

**本地技能（~/.hermes/skills/ 根目录）** — 5个，全部正常：

| Skill | SKILL.md | 大小 |
|-------|----------|------|
| article-short-story-review | ✅ | 221行 / 9.8KB |
| chinese-touch-ups | ✅ | 33行 / 2.1KB |
| dogfood | ✅ | 161行 / 6.3KB |
| video-prompting | ✅ | 118行 / 6.2KB |
| yuanbao | ✅ | 107行 / 3.8KB |

**分类目录下的技能子目录（hub/builtin）** — 160个子目录：
- ✅ 正常：154个
- ⚠️ **缺失/空 SKILL.md：6个**（全部在 `mlops/` 分类下）：

| 路径 | 问题 |
|------|------|
| `mlops/evaluation` | SKILL.md 缺失 |
| `mlops/inference` | SKILL.md 缺失 |
| `mlops/models` | SKILL.md 缺失 |
| `mlops/research` | SKILL.md 缺失 |
| `mlops/training` | SKILL.md 缺失 |
| `mlops/vector-databases` | SKILL.md 缺失 |

### ⚠️ 待确认

**mlops 分类下 6 个技能子目录无 SKILL.md**，可能是：
1. 这些是 hub 管理的分类子目录，本身不是独立 skill，SKILL.md 不在该层级（需确认是否为 hub 结构问题）
2. 或者是 skill 文件确实丢失

建议命令（确认是否为 hub 结构问题）：
```bash
# 查看这些目录的实际内容
ls -la ~/.hermes/skills/mlops/evaluation/
# 对比同分类下有 SKILL.md 的目录结构
ls -la ~/.hermes/skills/mlops/axolotl/
```

若确认需要 SKILL.md，修复命令：
```bash
# 从 hub 重新拉取或手动创建 SKILL.md
```

### ℹ️ 状态概览
- **本地技能**：5个，全部健康
- **分类技能子目录**：160个，154正常 / 6异常（均在 mlops/）
- **SOUL.md**：正常
- **config.yaml**：无 deprecated 模型
