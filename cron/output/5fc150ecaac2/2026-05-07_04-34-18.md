...
加载 daily-self-audit skill，按 Layer 2.2/2.3 执行 Skills 和配置审计：

Skills 审计（结构检查，不逐一 skill_view 深度审）：
1. 用 terminal 执行 `hermes skills list` 获取列表
2. 只检查 local skills 的 SKILL.md 是否存在且非空（用 read_file 或 cat）
3. 标记 SKILL.md 缺失或为空的 skill

配置审计：
1. 检查 ~/.hermes/SOUL.md 是否存在且非空
2. 检查 ~/.hermes/config.yaml 中是否有含 "deprecated" 的模型配置
...
#### Skills 审计 — 4个目录缺失 SKILL.md

| 目录 | 现状 | 风险 |
|------|------|------|
| `diagramming` | 仅有 DESCRIPTION.md (159 bytes) | 无法被 skill_view 加载，功能不可用 |
| `domain` | 仅有 DESCRIPTION.md (1142 bytes) | 同上 |
| `gifs` | 仅有 DESCRIPTION.md (109 bytes) | 同上 |
| `inference-sh` | 仅有 DESCRIPTION.md (706 bytes) | 同上 |

这4个目录结构异常——正常 skill 应有 SKILL.md 定义 metadata/tags/execution 等，它们只有 DESCRIPTION.md，属于**退化/半成品状态**。

**建议操作**：
- 若这些是废弃的 placeholder → 删除目录
- 若需要保留功能 → 补写 SKILL.md（参考 `~/.hermes/skills/creative/` 下其他 skill 的结构）
...
|--------|------|
| SOUL.md | ✅ 存在且非空 |
| config.yaml deprecated 模型 | ✅ 无 deprecated 配置 |
| config.yaml 关键段 | ✅ platforms、model、memory 均存在 |
...
|------|------|
| Skills 目录 | 32 个 |
| SKILL.md 文件 | 168 个（含嵌套） |
| 缺失 SKILL.md 的目录 | 4 个（diagramming/domain/gifs/inference-sh） |
| 配置健康度 | 正常 |
...
- 这4个目录只有 DESCRIPTION.md 没有 SKILL.md，说明它们从未被正确初始化过，不是后来丢失的——可能是 `hermes skill install` 失败或手动创建时遗漏
- 若这些目录实际被 `hermes skills list` 列为 enabled，说明系统把它们当 builtin 处理，但实际无法调用（无 SKILL.md 定义执行逻辑）
