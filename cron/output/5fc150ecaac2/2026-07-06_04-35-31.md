...
## 自审报告 2026-07-06 (Skills + Config)

### ✅ 已自动执行（Layer 0）
- **无** — 备份快照、缓存清理等由前序 cron 任务（02:30/03:03/03:33）负责，本任务仅审计。
...
**结构性异常：guizang-ppt-skill 注册但目录不在顶层**
- 注册名为 `guizang-ppt-skill`，实际目录位于 `~/.hermes/skills/creative/guizang-ppt-skill/`（子技能嵌套）
- `hermes skills list` 将其展示为独立顶层 skill，但 `ls skills/` 下无对应目录
- **这本身不是错误**（Hermes 可能通过 manifest 注册），只是路径不一致。如非预期，可确认是否需要迁移到顶层目录。

其他 15 个注册 skill 检查结果（SKILL.md 存在性 + 大小）：

| Skill | SKILL.md | 大小 |
|---|---|---|
| article-short-story-review | ✅ | 9.8K |
| chinese-touch-ups | ✅ | 2.1K |
| computer-use | ✅ | 11.2K |
| consumer-product-research | ✅ | 5.3K |
| cron-archive | ✅ | 6.1K |
| diagnose | ✅ | 1.3K |
| dogfood | ✅ | 6.3K |
| grill-me | ✅ | 3.4K |
| grill-with-docs | ✅ | 1.1K |
| handoff | ✅ | 920B |
| open-source-efficiency-tools | ✅ | 15.4K |
| open-source-efficiency-tools-research | ✅ | 7.4K |
| review | ✅ | 2.6K |
| tdd | ✅ | 844B |
| video-prompting | ✅ | 6.2K |

**目录噪声提示：** `~/.hermes/skills/` 下 50 个目录中，仅 18 个含 SKILL.md。其余 28 个（如 `apple/`, `creative/`, `devops/`, `data-science/` 等）实际包含子技能，空目录名不构成问题，但增加了浏览噪音。建议清理：
- 空目录（`diagramming/`, `domain/`, `inference-sh/`, `knowledge/`, `note-taking/` —— 各仅 1-2 个文件且无 SKILL.md）
- 参考命令（确认后执行）：
  ```bash
  for d in diagramming domain inference-sh knowledge note-taking gifs; do
    rm -rf ~/.hermes/skills/$d && echo "removed $d"
  done
  ```
...
| 检查项 | 结果 |
|---|---|
| SOUL.md 存在且非空 | ✅ 1.4K / 24 行 |
| config.yaml 关键段完整性 | ✅ platforms(2), custom_providers(3条), model(5键), memory(5键) |
| 废弃模型配置 | ✅ 无 — default: `deepseek-v4-flash`, provider: `custom` |
| config.yaml vs .bak 大小对比 | ✅ 当前 15.5K > .bak 7.7K（4月30日备份）。当前更大，无异常 |
| .bak 存在性 | ✅ 但备份距今 67 天，建议更新：`cp config.yaml config.yaml.bak` |
...
| 指标 | 值 |
|---|---|
| 注册 Skills | 16 个（均 enabled） |
| Skills 目录 | 50 个（18 含 SKILL.md，28 子技能/空壳，4 隐藏） |
| 配置完整性 | ✅ 无断裂段，无废弃模型 |
| SOUL.md | ✅ 正常 |
| config.yaml.bak 时效 | ⚠️ 67 天前，建议更新 |

---

### 📌 Pitfalls 交叉引用

- **config.bak 陈旧陷阱** — 当前 `.bak` 为 4月30日，大量新配置无备份。立即执行 `cp config.yaml config.yaml.bak` 更新
- **子技能嵌套陷阱** — `guizang-ppt-skill` 藏在 `creative/` 下但被 Hermes 注册为顶层 skill，删除 `creative/` 目录可能误删注册 skill
- **memory 工具批量操作陷阱** — 本审计未涉及 memory 修改，无需防范
- **配置恢复陷阱** — 若未来 config.yaml 损坏，当前 `.bak` 缺失 67 天的变更，恢复后需手动补充近期配置
